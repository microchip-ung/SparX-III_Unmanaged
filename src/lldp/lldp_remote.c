...
void lldp_remote_tlv_to_string (lldp_remote_entry_t xdata * entry, lldp_u8_t field, lldp_u8_t xdata * dest)
{
    ...
    case LLDP_TLV_ORG_EEE_TLV:
        if(entry->is_eee) {
            ...
            print_dec_nright(eee_sm[port2int(entry->receive_port)].tm.LocResolvedTxSystemValue, 14);
            print_dec_nright(eee_sm[port2int(entry->receive_port)].tm.LocResolvedRxSystemValue, 14);
        }
        break;
#endif
    default:
