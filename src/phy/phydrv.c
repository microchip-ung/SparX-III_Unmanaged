...
ushort phy_read (vtss_port_no_t port_no, uchar reg_no) small
{
    ...
    VTSS_F_DEVCPU_GCB_MIIM_MII_CMD_MIIM_CMD_PHYAD(phy_no)| /* Phy/port address */
    ...
    H2_READ(VTSS_DEVCPU_GCB_MIIM_MII_DATA(miim_no), dat);
    if (dat & VTSS_F_DEVCPU_GCB_MIIM_MII_DATA_MIIM_DATA_SUCCESS(3))
        return 0;
    return ((ushort) VTSS_X_DEVCPU_GCB_MIIM_MII_DATA_MIIM_DATA_RDDATA(dat));
}
               ...
               ushort           value) small
{
    ...
    /* Addressing part */
    dat |= VTSS_F_DEVCPU_GCB_MIIM_MII_CMD_MIIM_CMD_VLD | /* Valid command */
           VTSS_F_DEVCPU_GCB_MIIM_MII_CMD_MIIM_CMD_REGAD(reg_no) | /* Register address */
           VTSS_F_DEVCPU_GCB_MIIM_MII_CMD_MIIM_CMD_PHYAD(phy_no); /* Phy/port address */
...
void phy_write_masked(vtss_port_no_t port_no,
                      ...
                      ushort         mask) small
{
    value = (value & mask) | (phy_read(port_no, reg_no) & ~mask);
...
/* ************************************************************************ */
bool phy_link_status (vtss_port_no_t port_no) small
/* ------------------------------------------------------------------------ --
 ...
 */
void phy_do_link_up_settings (vtss_port_no_t    port_no,
                              uchar             link_mode,
                              uchar             power_mode)
{
    phy_id_t        phy_id;
    ...
    if ((link_mode & LINK_MODE_SPEED_MASK) == LINK_MODE_SPEED_1000)
        phy_receiver_reconfig(port_no, power_mode);
 ...
 */
void phy_receiver_reconfig( vtss_port_no_t  port_no,
                            uchar           power_mode )
{
    uchar       vga_state_a;
...
void phy_pre_reset (vtss_port_no_t port_no)
{
    ...
    switch (phy_id.family) {
#if 0//VTSS_COBRA not necessary for pre_reset
...
void phy_post_reset (vtss_port_no_t port_no)
{
...
#endif /* !defined(LUTON26_L16) && !defined(LUTON26_L16_QSGMII_EXT_PHY) */
    ...
    case VTSS_PHY_FAMILY_ELISE:
        ...
        if (port_no == 15) {
#ifdef PHY_DEBUG
            println_str("step 12. release coma_mode");
#endif
            // Release COMA_MODE after both internal and external PHYs are configured
            //     If COMA_MODE pin is connected between Luton26 and Atom12, only do
            //     this on one device; otherwise, you'll need to do it on both or use
...
{
    phy_id_t                phy_id;
#if (!defined(LUTON26_L10) && !defined(LUTON26_L16)) || VTSS_ELISE || VTSS_TESLA || VTSS_COBRA
    ...
    char                    *advertisements)
{
    ...
    reg_val         = phy_mmd_rd(port_no, 7, 61);
