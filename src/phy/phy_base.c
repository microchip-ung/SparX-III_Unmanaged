...
/****************************************************************************
 *                                                                          *
 *  Common functions that can be used by Microchip PHY chips                  *
 *                                                                          *
 ****************************************************************************/
...
vtss_rc vtss_phy_wait_for_micro_complete(vtss_port_no_t port_no)
{
    ...
    while (reg18g & 0x8000 && timeout > 0)
    {
        ...
        VTSS_MSLEEP(1);
//        VTSS_D("MCB timer, port_no %lu, timeout = %d", port_no ,timeout);
    }
...
#if VTSS_ATOM12 || VTSS_TESLA || VTSS_ELISE
/**
...
#if VTSS_ATOM12_B || VTSS_ATOM12_C || VTSS_ATOM12_D || VTSS_TESLA || VTSS_ELISE
#if TRANSIT_VERIPHY
...
vtss_rc vtss_phy_soft_reset_port (
    ...
    while (loops--) {
        VTSS_MSLEEP(5);                  /* pause after reset */
