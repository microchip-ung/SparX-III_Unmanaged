...
#if LUTON_UNMANAGED_CONF_IF || defined(TRANSIT_WEB)
/*
...
#endif /* LUTON_UNMANAGED_CONF_IF || defined(TRANSIT_WEB) */
...
{
    lldp_port_t iport = port2int(port_no);
...
void lldp_tx_frame (lldp_port_t port_no)
{
    ...
    if ((mac_addr.macaddr[5] += port_no) < (port_no)) {
        if (++mac_addr.macaddr[4] == 0) {
            mac_addr.macaddr[3]++;
        }
    }
...
{
    return lldp_sm[port2int(port)].stats.statsFramesOutTotal;
}

lldp_counter_t lldp_get_rx_total_frames (lldp_port_t port)
{
    return lldp_sm[port2int(port)].stats.statsFramesInTotal;
}

lldp_counter_t lldp_get_rx_error_frames (lldp_port_t port)
{
    return lldp_sm[port2int(port)].stats.statsFramesInErrorsTotal;
}

lldp_counter_t lldp_get_rx_discarded_frames (lldp_port_t port)
{
    return lldp_sm[port2int(port)].stats.statsFramesDiscardedTotal;
}

lldp_counter_t lldp_get_TLVs_discarded (lldp_port_t port)
{
    return lldp_sm[port2int(port)].stats.statsTLVsDiscardedTotal;
}

lldp_counter_t lldp_get_TLVs_unrecognized (lldp_port_t port)
{
    return lldp_sm[port2int(port)].stats.statsTLVsUnrecognizedTotal;
}

lldp_counter_t lldp_get_TLVs_org_discarded (lldp_port_t port)
{
    return lldp_sm[port2int(port)].stats.statsOrgTVLsDiscarded;
}

lldp_counter_t lldp_get_ageouts (lldp_port_t port)
{
    return lldp_sm[port2int(port)].stats.statsAgeoutsTotal;
